...
from shop.models import User, AddProduct, Brand, Category, UserCustomer, CustomerOrder
...
def customer_register():
    ...
    if form.validate_on_submit():
        ...
        register = UserCustomer(username=form.username.data, email=form.email.data,password=hash_password,country=form.country.data, city=form.city.data,contact=form.contact.data, address=form.address.data)
...
def customerLogin():
    ...
    if form.validate_on_submit():
        user = UserCustomer.query.filter_by(email=form.email.data).first()
...
def orders(invoice):
    if current_user.is_authenticated:
        ...
        subTotal = 0
        customer_id = current_user.id
        customer = UserCustomer.query.filter_by(id=customer_id).first()
